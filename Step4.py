...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 27 14:28:11 2018
...
for line in f:
    #line = f.readline()
    tmp = line.split('\t')
    title_train.append(tmp[1]+' '+ tmp[4].strip('\n'))
...
f2 = open("text_hin_features.txt", "w")

title = []
lb = []
for line in f:
    tmp = line.split('\t')
    feature = X.transform([tmp[1]+' '+ tmp[4].strip('\n')]).toarray()[0].tolist()
...
for line in f_vali:
    tmp = line.split('\t')
    title_vali.append(tmp[1]+' '+ tmp[4].strip('\n'))
...
f = open("precision_recall_hin.txt", "w")
...
for line in f_test:
    ...
    title_test.append(tmp[1]+' '+ tmp[4].strip('\n'))
f_test.close()

x_test = X.transform(title_test)
pred_test = clf.predict(x_test)

f = open("text_hin_feature_predictions.txt", "w")
